...
A configurable **Temporary Workspace** for DeepSeek Harness. It is intended for one-off analysis and plugin-owned background work that must stay outside project directories. The plugin keeps one Workspace at the bottom of the list; every temporary Session is accounted under that group instead of spilling into Ungrouped. Use ordinary Workspace controls when a task belongs to a project.
...
2. The Host creates or resolves the native Harness Workspace registration; the Client applies the localized Temporary Workspace label while preserving a later user-customized title.
...
The same value is editable live under **Settings → Plugins → Temporary Workspace**. The card accepts an absolute path and, when the Host uses the native directory-picker backend, provides a **Choose directory** button. Saving creates and permission-checks the directory; the next temporary Session registers and uses the new Workspace. Existing Workspaces, Sessions, and files are neither migrated nor deleted. **Use default** restores the composition value above, which is `$DSH_HOME/temporary-sessions` (normally `~/.dsh/temporary-sessions`) unless the profile overrides `root`.
